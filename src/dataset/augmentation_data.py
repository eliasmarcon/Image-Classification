...
def create_testing_batch(number_of_images: int = 30) -> None:
    ...
    if os.path.exists("./testing_dataset/"):
        print("The testing_dataset folder already exists and therefore no additional images will be added")
        ...
        for folder in FOLDER_TYPES:
            os.mkdir(f"./testing_dataset/{folder}")
            
        # take X images from each folder randomly and move them to the testing_dataset folder
        for folder in FOLDER_TYPES:
            ...
            for i in range(number_of_images):
                ...
                shutil.move(temp_images.pop(0), f"./testing_dataset/{folder}/{i}.jpg")
...
def plot_augmented_images(number_of_images: int = 3) -> None:
    ...
    plt.tight_layout()
    plt.savefig("./readme_images/augmented_images.png")
